...
function Container(container_directory, package_json) {
    ...
    let toc_yaml = {};
    try {
        config_yaml = YAML.parse(fs.readFileSync(path.join(container_directory, content_dir, 'config.yml')));
    } catch (err){
        console.warn(err);
    }
    try {
        toc_yaml = YAML.parse(fs.readFileSync(path.join(container_directory, content_dir, 'toc.yml')));
    } catch (err){
        console.warn(err);
    }
                        ...
                        for (let chapter of data.chapters) {
                            ...
                            for (let frame of chapter.frames) {
                                ...
                                if(props.tw_assignments) {
                                    try {
                                        words = props.tw_assignments[chapter.number][frameSlug];
                                    } catch(e) {}
                                }
