...
function loadContainer(container_directory) {
    return new Promise(function(resolve, reject) {
        ...
        if(package_json == null) {
            reject(new Error('Invalid resource container. Missing package.json'));
            return;
        } else if(package_json.package_version == null) {
            reject(new Error('Invalid resource container. Missing field "package_version".'));
            return;
        }
        if (compareVersions(package_json.package_version, spec.version) === 1) {
            reject(new Error('Unsupported resource container version. This library supports version ' + spec.version + ' but was given ' + package_json.package_version));
            return;
        }
        if(compareVersions(package_json.package_version, spec.version) === -1) {
            reject(new Error('Outdated resource container version. This library supports version ' + spec.version + ' but was given ' + package_json.package_version));
            return;
        }
