                ...
                try {
                    ...
                    if(props.resource.type === 'book') {
                        ...
                        for (let chapter of data.chapters) {
                            ...
                            if (chapter.title) {
                                config.content[chapter.number]['title'] = {};
                            ...
                            for (let frame of chapter.frames) {
                                ...
                                // TODO: need to add words, questions, notes, and images to the config for the chunk
                                config.content[chapter.number][frameSlug] = {};
                                chapterTOC.chunks.push(frameSlug);
                                fs.writeFileSync(path.join(chapterDir, frameSlug + '.' + chunk_ext), frame.text);
                            }
                            if (chapter.ref) {
                                config.content[chapter.number]['reference'] = {};
                    ...
                    fs.writeFileSync(configPath, new Buffer(YAML.stringify(config, 6, 2, null, null)));
                    // write toc
                    let tocPath = path.join(contentDir, 'toc.yml');
                    fs.writeFileSync(tocPath, new Buffer(YAML.stringify(toc, 6, 2, null, null)));
                } catch(err) {
                    console.error(data);
                    reject(err);
                    return;
